...
//#define RTP_PAYLOAD_TYPE "114"
#define RTP_PAYLOAD_TYPE "102"

#define SOUP_HTTP_PORT 8000
//#define STUN_SERVER "nong.be:3478"
#define STUN_SERVER "turnhk.nong.be:3478"
...
<html> \n \
 ...
 \n \
	...
	html5VideoElement.autoplay = true; \n \
        html5VideoElement.controls = true; \n \
 ...
 \n \
	...
	var config = { 'iceServers': [{'urls': 'stun:turnhk.nong.be:3478'},{'urls': 'turn:turnhk.nong.be:5349','username': 'frodoturn_ph','credential': 'sambros2021'}] } \n \
...
// var wsUrl = \"ws://dev.nong.be:8000/server\"; \n \
...
create_receiver_entry (SoupWebsocketConnection * connection)
{
  ...
  receiver_entry->pipeline = gst_parse_launch ("webrtcbin name=webrtcbin stun-server=stun://" STUN_SERVER " "
      "nvarguscamerasrc ! video/x-raw(memory:NVMM), format=NV12, width=1640, height=1232 ! nvvidconv flip-method=2 ! nvvidconv ! video/x-raw(memory:NVMM), width=1280, height=960 ! nvvidconv ! videorate ! video/x-raw, framerate=12/1 ! omxh264enc bitrate=2200000 peak-bitrate=2700000 control-rate=1 vbv-size=10 profile=1 ! video/x-h264, stream-format=(string)byte-stream ! queue max-size-time=100000000 ! h264parse ! "
      "rtph264pay config-interval=-1 seqnum-offset=0 timestamp-offset=0 mtu=1300 name=payloader ! "
